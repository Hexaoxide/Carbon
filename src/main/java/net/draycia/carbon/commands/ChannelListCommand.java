...
public class ChannelListCommand {
  ...
  public void executeSelf(@NonNull Player player, @NonNull Object @NonNull [] args) {
    Iterator<ChatChannel> allChannels =
        carbonChat.getChannelManager().getRegistry().values().iterator();
  ...
  private void getListAndSend(
      @NonNull CommandSender sender,
      @NonNull ChatUser user,
      @NonNull Iterator<ChatChannel> allChannels) {
    ChatChannel channel;
    ...
    Component availableComponent =
        carbonChat.getAdventureManager().processMessage(availableFormat, "br", "\n");
    availableComponent =
        availableComponent.replaceFirstText(
            Pattern.compile(Pattern.quote("<list>")), (ac) -> availableList);
    ...
    if (sender.hasPermission("carbonchat.channellist.bypass") && !cannotSee.isEmpty()) {
      ...
      Component unavailableComponent =
          carbonChat.getAdventureManager().processMessage(unavailableFormat, "br", "\n");
      unavailableComponent =
          unavailableComponent.replaceFirstText(
              Pattern.compile(Pattern.quote("<list>")), (uac) -> unavailableList);
  ...
  private void makeList(
      @NonNull Iterator<@NonNull ChatChannel> iterator, TextComponent.@NonNull Builder list) {
