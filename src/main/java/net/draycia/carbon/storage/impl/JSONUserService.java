...
public class JSONUserService implements UserService {
  ...
  @NonNull private final Gson gson = new GsonBuilder().setPrettyPrinting().create();
  @NonNull private final Type userType = new TypeToken<CarbonChatUser>() {}.getType();
  ...
  public JSONUserService(@NonNull CarbonChat carbonChat) {
    ...
    Bukkit.getScheduler()
        .scheduleSyncRepeatingTask(carbonChat, userCache::cleanUp, 0L, 20 * 60 * 10);
  }
  ...
  private void saveUser(
    ...
    File userFile =
        new File(carbonChat.getDataFolder(), "users/" + notification.getKey().toString() + ".json");
    ensureFileExists(userFile);
