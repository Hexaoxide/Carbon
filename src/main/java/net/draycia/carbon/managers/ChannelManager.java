...
public class ChannelManager {
  ...
  @NonNull private final ChannelRegistry registry;

  @Nullable @MonotonicNonNull private String defaultChannelKey = null;
  ...
  public boolean registerChannel(@NonNull ChatChannel channel) {
    ...
    if (success) {
      ...
      Bukkit.getPluginManager()
          .callEvent(new ChannelRegisterEvent(Collections.singletonList(channel), getRegistry()));
    }
  ...
  private void reload() {
    ...
    for (String key : carbonChat.getConfig().getConfigurationSection("channels").getKeys(false)) {
      ConfigurationSection section =
          carbonChat.getConfig().getConfigurationSection("channels").getConfigurationSection(key);
